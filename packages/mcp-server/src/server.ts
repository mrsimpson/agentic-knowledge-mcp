...
export function createAgenticKnowledgeServer() {
  ...
  let configCache: { config: KnowledgeConfig; configPath: string } | null = null;
   ...
   */
  async function getConfiguration(): Promise<{ config: KnowledgeConfig; configPath: string }> {
    const now = Date.now();
  ...
  server.setRequestHandler(ListToolsRequestSchema, async () => {
    return {
      tools: [
        {
          name: 'search_docs',
          description: 'Search for documentation guidance based on keywords and context. Returns intelligent navigation instructions to help you find relevant information in a specific docset.',
          inputSchema: {
            type: 'object',
            properties: {
              docset_id: {
