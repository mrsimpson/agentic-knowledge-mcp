...
describe("Configuration Loading and Validation", () => {
  ...
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(
      join(tmpdir(), "agentic-knowledge-loader-test-"),
    );
...
template: "Global template with {{keywords}} and {{invalid_variable}}"`;
    ...
    test("should throw TEMPLATE_ERROR for invalid docset template", async () => {
      ...
      const invalidDocsetTemplateConfigPath = join(
        tempDir,
        "invalid-docset-template.yaml",
      );
...
docsets:
  - id: "test-docs"
    ...
    template: "Docset template missing {{keywords}} but has {{bad_var}}"`;
      ...
      await expect(loadConfig(invalidDocsetTemplateConfigPath)).rejects.toThrow(
        KnowledgeError,
      );
