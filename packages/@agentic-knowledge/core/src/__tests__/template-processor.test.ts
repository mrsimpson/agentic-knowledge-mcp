...
import { KnowledgeError, ErrorType, DEFAULT_TEMPLATE, ALLOWED_TEMPLATE_VARIABLES } from '../types.js';
...
describe('Template Processing', () => {
  ...
  describe('processTemplate - core functionality', () => {
    test('should replace all valid template variables', () => {
      ...
      expect(result).toBe('Search "react hooks" in /project/docs (Test Documentation). Also try: react state management. Description: Sample documentation for testing');
    });
    ...
    test('should throw error for invalid variables in processed template', () => {
      ...
      expect(() => processTemplate(template, sampleContext)).toThrow(KnowledgeError);
      ...
      try {
        ...
        expect((error as KnowledgeError).message).toContain('invalid variables');
      }
    });
  ...
  describe('validateTemplate - validation at startup', () => {
    ...
    test('should reject template with invalid variables', () => {
      ...
      try {
        ...
        expect(error).toBeInstanceOf(KnowledgeError);
        expect((error as KnowledgeError).type).toBe(ErrorType.TEMPLATE_ERROR);
        expect((error as KnowledgeError).message).toContain('invalid variables: invalid_variable');
        expect((error as KnowledgeError).message).toContain('Allowed variables:');
      }
    });
  ...
  describe('extractVariables - parsing functionality', () => {
    test('should extract all variables from complex template', () => {
      const template = 'Search {{keywords}} in {{local_path}} for {{docset_name}} ({{docset_id}})';
      
      const result = extractVariables(template);
      
      expect(result).toEqual(['keywords', 'local_path', 'docset_name', 'docset_id']);
    });
  ...
  describe('getEffectiveTemplate - template resolution', () => {
    test('should prioritize docset template over global and default', () => {
      ...
      const result = getEffectiveTemplate(docsetWithTemplate, 'Global template');
