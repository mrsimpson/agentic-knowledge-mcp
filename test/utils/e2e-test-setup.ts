 ...
 */
export async function createTestProject(config: E2ETestConfig): Promise<TestProject> {
  const projectPath = await fs.mkdtemp(join(tmpdir(), `agentic-knowledge-e2e-${config.testName}-`));
