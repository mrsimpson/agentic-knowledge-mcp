...
describe("MCP Protocol Compliance E2E Tests", () => {
  ...
  describe("Tool Schema Validation", () => {
    ...
    it("should have correct search_docs tool schema", async () => {
      const tools = await client.listTools();
      const searchDocsTool = tools.tools.find(
      ...
      expect(properties.docset_id.description).toContain(
    ...
    it("should have correct list_docsets tool schema", async () => {
      const tools = await client.listTools();
      const listDocsetsTool = tools.tools.find(
        (tool) => tool.name === "list_docsets",
      );

      expect(listDocsetsTool).toBeDefined();
      expect(listDocsetsTool!.description).toContain(
